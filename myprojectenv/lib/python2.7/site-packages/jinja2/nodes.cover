    ...
    1: from jinja2.utils import Markup
    1: from jinja2._compat import izip, with_metaclass, text_type
       
       
       #: the types we support for context functions
    1: _context_function_types = (types.FunctionType, types.MethodType)
    ...
    1:     def __new__(cls, name, bases, d):
  192:         for attr in 'fields', 'attributes':
  128:             storage = []
  128:             storage.extend(getattr(bases[0], attr, ()))
  128:             storage.extend(d.get(attr, ()))
  128:             assert len(bases) == 1, 'multiple inheritance not allowed'
  128:             assert len(storage) == len(set(storage)), 'layout conflict'
  128:             d[attr] = tuple(storage)
   64:         d.setdefault('abstract', False)
   64:         return type.__new__(cls, name, bases, d)
   ...
   18:         ', '.join(sorted('``%s``' % x for x in set(_binop_to_func) |
    1:                   set(_uaop_to_func) | set(_cmpop_to_func)))
    ...
    1: NodeType.__new__ = staticmethod(_failing_new); del _failing_new
