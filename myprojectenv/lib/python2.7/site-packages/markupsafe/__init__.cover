    ...
    1: from markupsafe._compat import text_type, string_types, int_types, \
    ...
    1:     def make_simple_escaping_wrapper(name):
   17:         orig = getattr(text_type, name)
   17:         def func(self, *args, **kwargs):
                   args = _escape_argspec(list(args), enumerate(args), self.escape)
                   _escape_argspec(kwargs, iteritems(kwargs), self.escape)
                   return self.__class__(orig(self, *args, **kwargs))
   17:         func.__name__ = orig.__name__
   17:         func.__doc__ = orig.__doc__
   17:         return func
       
    1:     for method in '__getitem__', 'capitalize', \
                         'title', 'lower', 'upper', 'replace', 'ljust', \
                         'rjust', 'lstrip', 'rstrip', 'center', 'strip', \
   17:                   'translate', 'expandtabs', 'swapcase', 'zfill':
   16:         locals()[method] = make_simple_escaping_wrapper(method)
       
           # new in python 2.5
    1:     if hasattr(text_type, 'partition'):
    ...
    1:         __getslice__ = make_simple_escaping_wrapper('__getslice__')
    ...
    1:     __getitem__ = lambda s, x: _MarkupEscapeHelper(s.obj[x], s.escape)
    1:     __unicode__ = __str__ = lambda s: text_type(s.escape(s.obj))
    1:     __repr__ = lambda s: str(s.escape(repr(s.obj)))
    1:     __int__ = lambda s: int(s.obj)
    1:     __float__ = lambda s: float(s.obj)
       
       
       # we have to import it down here as the speedups and native
       # modules imports the markup type which is define above.
    1: try:
    1:     from markupsafe._speedups import escape, escape_silent, soft_unicode
       except ImportError:
