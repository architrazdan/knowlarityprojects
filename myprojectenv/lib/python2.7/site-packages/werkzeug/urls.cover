    ...
    1: import os
    1: import re
    1: from werkzeug._compat import text_type, PY2, to_unicode, \
           to_native, implements_to_string, try_coerce_native, \
           normalize_string_tuple, make_literal_wrapper, \
           fix_tuple_repr
    1: from werkzeug._internal import _encode_idna, _decode_idna
    1: from werkzeug.datastructures import MultiDict, iter_multi_items
    1: from collections import namedtuple
       
       
       # A regular expression for what a valid schema looks like
    1: _scheme_re = re.compile(r'^[a-zA-Z0-9+-.]+$')
       
       # Characters that are safe in any part of an URL.
    1: _always_safe = (b'abcdefghijklmnopqrstuvwxyz'
                       b'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_.-+')
       
    1: _hexdigits = '0123456789ABCDEFabcdef'
    1: _hextobyte = dict(
  508:     ((a + b).encode(), int(a + b, 16))
    1:     for a in _hexdigits for b in _hexdigits
       )
       
       
    1: _URLTuple = fix_tuple_repr(namedtuple(
    1:     '_URLTuple',
    1:     ['scheme', 'netloc', 'path', 'query', 'fragment']
       ))
       
       
    2: class BaseURL(_URLTuple):
       
    1:     '''Superclass of :py:class:`URL` and :py:class:`BytesURL`.'''
    1:     __slots__ = ()
    ...
    1:     def decode_query(self, *args, **kwargs):
    ...
    1:     def get_file_location(self, pathformat=None):
    ...
    1:     def encode(self, charset='utf-8', errors='replace'):
    ...
    2: class BytesURL(BaseURL):
       
    1:     """Represents a parsed URL in bytes."""
    1:     __slots__ = ()
    1:     _at = b'@'
    1:     _colon = b':'
    1:     _lbracket = b'['
    ...
    1:     def decode(self, charset='utf-8', errors='replace'):
    ...
    1: def _unquote_to_bytes(string, unsafe=''):
    5:     if isinstance(string, text_type):
               string = string.encode('utf-8')
    5:     if isinstance(unsafe, text_type):
               unsafe = unsafe.encode('utf-8')
    5:     unsafe = frozenset(bytearray(unsafe))
    5:     bits = iter(string.split(b'%'))
    5:     result = bytearray(next(bits, b''))
    5:     for item in bits:
               try:
    ...
    1: def _url_encode_impl(obj, charset, encode_keys, sort, key):
    ...
    1: def _url_unquote_legacy(value, unsafe=''):
           try:
    ...
    1: def url_parse(url, scheme=None, allow_fragments=True):
           ...
           :param allow_fragments: if set to `False` a fragment will be removed
                                   from the URL.
           """
    5:     s = make_literal_wrapper(url)
    5:     is_text_based = isinstance(url, text_type)
       
    5:     if scheme is None:
    5:         scheme = s('')
    5:     netloc = query = fragment = s('')
    5:     i = url.find(s(':'))
    5:     if i > 0 and _scheme_re.match(to_native(url[:i], errors='replace')):
    ...
    5:     if allow_fragments and s('#') in url:
               url, fragment = url.split(s('#'), 1)
    5:     if s('?') in url:
               url, query = url.split(s('?'), 1)
       
    5:     result_type = is_text_based and URL or BytesURL
    5:     return result_type(scheme, netloc, url, query, fragment)
       
       
    1: def url_quote(string, charset='utf-8', errors='strict', safe='/:', unsafe=''):
    ...
    1: def url_quote_plus(string, charset='utf-8', errors='strict', safe=''):
    ...
    1: def url_unquote(string, charset='utf-8', errors='replace', unsafe=''):
           ...
           """
    5:     rv = _unquote_to_bytes(string, unsafe)
    5:     if charset is not None:
    5:         rv = rv.decode(charset, errors)
    5:     return rv
       
       
    1: def url_unquote_plus(s, charset='utf-8', errors='replace'):
    ...
    1: def uri_to_iri(uri, charset='utf-8', errors='replace'):
           r"""
    ...
    1: def iri_to_uri(iri, charset='utf-8', errors='strict', safe_conversion=False):
           r"""
    ...
    1: def url_decode(s, charset='utf-8', decode_keys=False, include_empty=True,
    1:                errors='replace', separator='&', cls=None):
           """
    ...
    5:     if cls is None:
               cls = MultiDict
    5:     if isinstance(s, text_type) and not isinstance(separator, text_type):
               separator = separator.decode(charset or 'ascii')
    5:     elif isinstance(s, bytes) and not isinstance(separator, bytes):
               separator = separator.encode(charset or 'ascii')
    5:     return cls(_url_decode_impl(s.split(separator), charset, decode_keys,
    5:                                 include_empty, errors))
       
       
    1: def url_decode_stream(stream, charset='utf-8', decode_keys=False,
    1:                       include_empty=True, errors='replace', separator='&',
    1:                       cls=None, limit=None, return_iterator=False):
    ...
    1: def _url_decode_impl(pair_iter, charset, decode_keys, include_empty, errors):
   10:     for pair in pair_iter:
    5:         if not pair:
    5:             continue
    ...
    1: def url_encode(obj, charset='utf-8', encode_keys=False, sort=False, key=None,
    1:                separator=b'&'):
    ...
    1: def url_encode_stream(obj, stream=None, charset='utf-8', encode_keys=False,
    1:                       sort=False, key=None, separator=b'&'):
    ...
    1: def url_join(base, url, allow_fragments=True):
    ...
    1:     def __init__(self, base='./', charset='utf-8', sort=False, key=None):
               if not base:
                   base = './'
               self.base = base
    ...
    1:     def __call__(self, *path, **query):
