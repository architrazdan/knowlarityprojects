    ...
    1: try:
    1:     from urllib2 import Request as U2Request
       except ImportError:
           from urllib.request import Request as U2Request
    1: try:
    1:     from http.cookiejar import CookieJar
    1: except ImportError:  # Py2
    1:     from cookielib import CookieJar
       
    1: from werkzeug._compat import iterlists, iteritems, itervalues, to_bytes, \
           string_types, text_type, reraise, wsgi_encoding_dance, \
           make_literal_wrapper
    1: from werkzeug._internal import _empty_stream, _get_environ
    1: from werkzeug.wrappers import BaseRequest
    1: from werkzeug.urls import url_encode, url_fix, iri_to_uri, url_unquote, \
           url_unparse, url_parse
    1: from werkzeug.wsgi import get_host, get_current_url, ClosingIterator
    1: from werkzeug.utils import dump_cookie
    1: from werkzeug.datastructures import FileMultiDict, MultiDict, \
           CombinedMultiDict, Headers, FileStorage
       
       
    1: def stream_encode_multipart(values, use_tempfile=True, threshold=1024 * 500,
    1:                             boundary=None, charset='utf-8'):
    ...
    1: def encode_multipart(values, boundary=None, charset='utf-8'):
    ...
    1: def File(fd, filename=None, mimetype=None):
           """Backwards compat."""
    ...
    2: class _TestCookieHeaders(object):
    ...
    1:     def get_all(self, name, default=None):
               rv = []
               for k, v in self.headers:
    ...
    2: class _TestCookieResponse(object):
    ...
    2: class _TestCookieJar(CookieJar):
       
           """A cookielib.CookieJar modified to inject and read cookie headers from
           and to wsgi environments, and wsgi application responses.
    1:     """
       
    1:     def inject_wsgi(self, environ):
    ...
    1:     def extract_wsgi(self, environ, headers):
    ...
    1:     request_class = BaseRequest
       
    1:     def __init__(self, path='/', base_url=None, query_string=None,
    1:                  method='GET', input_stream=None, content_type=None,
    1:                  content_length=None, errors_stream=None, multithread=False,
    1:                  multiprocess=False, run_once=False, headers=None, data=None,
    1:                  environ_base=None, environ_overrides=None, charset='utf-8'):
    ...
    1:     def _add_file_from_data(self, key, value):
    ...
    1:     base_url = property(_get_base_url, _set_base_url, doc='''
               The base URL is a URL that is used to extract the WSGI
               URL scheme, host (server name + server port) and the
    1:         script root (`SCRIPT_NAME`).''')
    1:     del _get_base_url, _set_base_url
       
    1:     def _get_content_type(self):
    ...
    1:     def _set_content_type(self, value):
               if value is None:
                   self.headers.pop('Content-Type', None)
               else:
                   self.headers['Content-Type'] = value
       
    1:     content_type = property(_get_content_type, _set_content_type, doc='''
               The content type for the request.  Reflected from and to the
               :attr:`headers`.  Do not set if you set :attr:`files` or
    1:         :attr:`form` for auto detection.''')
    1:     del _get_content_type, _set_content_type
       
    1:     def _get_content_length(self):
               return self.headers.get('Content-Length', type=int)
       
    1:     def _set_content_length(self, value):
               if value is None:
                   self.headers.pop('Content-Length', None)
               else:
                   self.headers['Content-Length'] = str(value)
       
    1:     content_length = property(_get_content_length, _set_content_length, doc='''
               The content length as integer.  Reflected from and to the
               :attr:`headers`.  Do not set if you set :attr:`files` or
    1:         :attr:`form` for auto detection.''')
    1:     del _get_content_length, _set_content_length
       
    1:     def form_property(name, storage, doc):
    2:         key = '_' + name
    ...
    2:         def setter(self, value):
                   self._input_stream = None
                   setattr(self, key, value)
    2:         return property(getter, setter, doc)
       
    1:     form = form_property('form', MultiDict, doc='''
    1:         A :class:`MultiDict` of form values.''')
    1:     files = form_property('files', FileMultiDict, doc='''
               A :class:`FileMultiDict` of uploaded files.  You can use the
               :meth:`~FileMultiDict.add_file` method to add new files to the
    1:         dict.''')
    1:     del form_property
       
    1:     def _get_input_stream(self):
               return self._input_stream
       
    1:     def _set_input_stream(self, value):
               self._input_stream = value
               self._form = self._files = None
       
    1:     input_stream = property(_get_input_stream, _set_input_stream, doc='''
               An optional input stream.  If you set this it will clear
    1:         :attr:`form` and :attr:`files`.''')
    1:     del _get_input_stream, _set_input_stream
       
    1:     def _get_query_string(self):
    ...
    1:     def _set_query_string(self, value):
               self._query_string = value
               self._args = None
       
    1:     query_string = property(_get_query_string, _set_query_string, doc='''
               The query string.  If you set this to a string :attr:`args` will
    1:         no longer be available.''')
    1:     del _get_query_string, _set_query_string
    ...
    1:     def _set_args(self, value):
               self._query_string = None
               self._args = value
       
    1:     args = property(_get_args, _set_args, doc='''
    1:         The URL arguments as :class:`MultiDict`.''')
    1:     del _get_args, _set_args
    ...
    1:     def get_request(self, cls=None):
    ...
    2: class ClientRedirectError(Exception):
    ...
    1:     def __init__(self, application, response_wrapper=None, use_cookies=True,
    1:                  allow_subdomain_redirects=False):
    ...
    1:     def set_cookie(self, server_name, key, value='', max_age=None,
    1:                    expires=None, path='/', domain=None, secure=None,
    1:                    httponly=False, charset='utf-8'):
    ...
    1:     def delete_cookie(self, server_name, key, path='/', domain=None):
               """Deletes a cookie in the test client."""
               self.set_cookie(server_name, key, expires=0, max_age=0,
                               path=path, domain=domain)
       
    1:     def run_wsgi_app(self, environ, buffered=False):
    ...
    1:     def resolve_redirect(self, response, new_location, environ, buffered=False):
    ...
    1:     def get(self, *args, **kw):
               """Like open but method is enforced to GET."""
               kw['method'] = 'GET'
               return self.open(*args, **kw)
       
    1:     def patch(self, *args, **kw):
               """Like open but method is enforced to PATCH."""
               kw['method'] = 'PATCH'
               return self.open(*args, **kw)
       
    1:     def post(self, *args, **kw):
               """Like open but method is enforced to POST."""
               kw['method'] = 'POST'
               return self.open(*args, **kw)
       
    1:     def head(self, *args, **kw):
               """Like open but method is enforced to HEAD."""
               kw['method'] = 'HEAD'
               return self.open(*args, **kw)
       
    1:     def put(self, *args, **kw):
               """Like open but method is enforced to PUT."""
               kw['method'] = 'PUT'
               return self.open(*args, **kw)
       
    1:     def delete(self, *args, **kw):
               """Like open but method is enforced to DELETE."""
               kw['method'] = 'DELETE'
               return self.open(*args, **kw)
       
    1:     def options(self, *args, **kw):
               """Like open but method is enforced to OPTIONS."""
               kw['method'] = 'OPTIONS'
               return self.open(*args, **kw)
       
    1:     def trace(self, *args, **kw):
    ...
    1: def create_environ(*args, **kwargs):
    ...
    1: def run_wsgi_app(app, environ, buffered=False):
           ...
           """
    4:     environ = _get_environ(environ)
    4:     response = []
    4:     buffer = []
       
    4:     def start_response(status, headers, exc_info=None):
    4:         if exc_info is not None:
                   reraise(*exc_info)
    4:         response[:] = [status, headers]
    4:         return buffer.append
       
    4:     app_rv = app(environ, start_response)
    4:     close_func = getattr(app_rv, 'close', None)
    4:     app_iter = iter(app_rv)
    ...
    4:         while not response:
                   buffer.append(next(app_iter))
    4:         if buffer:
                   app_iter = chain(buffer, app_iter)
    4:         if close_func is not None and app_iter is not app_rv:
                   app_iter = ClosingIterator(app_iter, close_func)
       
    4:     return app_iter, response[0], Headers(response[1])
