    ...
    1: from werkzeug._compat import iter_bytes, text_type, BytesIO, int_to_byte, \
           range_type, integer_types
       
       
    1: _logger = None
    1: _empty_stream = BytesIO()
    1: _signature_cache = WeakKeyDictionary()
    1: _epoch_ord = date(1970, 1, 1).toordinal()
    1: _cookie_params = set((b'expires', b'path', b'comment',
    ...
    1: _cookie_quoting_map = {
    1:     b',': b'\\054',
    1:     b';': b'\\073',
    1:     b'"': b'\\"',
    1:     b'\\': b'\\\\',
       }
  162: for _i in chain(range_type(32), range_type(127, 256)):
  161:     _cookie_quoting_map[int_to_byte(_i)] = ('\\%03o' % _i).encode('latin1')
       
       
    1: _octal_re = re.compile(b'\\\\[0-3][0-7][0-7]')
    1: _quote_re = re.compile(b'[\\\\].')
    1: _legal_cookie_chars_re = b'[\w\d!#%&\'~_`><@,:/\$\*\+\-\.\^\|\)\(\?\}\{\=]'
    1: _cookie_re = re.compile(b"""(?x)
           (?P<key>[^=]+)
           \s*=\s*
           (?P<val>
               "(?:[^\\\\"]|\\\\.)*" |
    ...
    1: def _get_environ(obj):
   13:     env = getattr(obj, 'environ', obj)
   13:     assert isinstance(env, dict), \
               '%r is not a WSGI environment (has to be a dict)' % type(obj).__name__
   13:     return env
       
       
    1: def _log(type, message, *args, **kwargs):
           """Log into the internal werkzeug logger."""
           global _logger
    6:     if _logger is None:
    1:         import logging
    1:         _logger = logging.getLogger('werkzeug')
               # Only set up a default log handler if the
               # end-user application didn't set anything up.
    1:         if not logging.root.handlers and _logger.level == logging.NOTSET:
    1:             _logger.setLevel(logging.INFO)
    1:             handler = logging.StreamHandler()
    1:             _logger.addHandler(handler)
    6:     getattr(_logger, type)(message.rstrip(), *args, **kwargs)
       
       
    1: def _parse_signature(func):
    ...
    2: class _DictAccessorProperty(object):
       
    1:     """Baseclass for `environ_property` and `header_property`."""
    1:     read_only = False
       
    1:     def __init__(self, name, default=None, load_func=None, dump_func=None,
    1:                  read_only=None, doc=None):
   25:         self.name = name
   25:         self.default = default
   25:         self.load_func = load_func
   25:         self.dump_func = dump_func
   25:         if read_only is not None:
    2:             self.read_only = read_only
   25:         self.__doc__ = doc
       
    1:     def __get__(self, obj, type=None):
   14:         if obj is None:
                   return self
   14:         storage = self.lookup(obj)
   14:         if self.name not in storage:
                   return self.default
   14:         rv = storage[self.name]
   14:         if self.load_func is not None:
   14:             try:
   14:                 rv = self.load_func(rv)
   ...
   14:         return rv
    ...
    1: def _encode_idna(domain):
           # If we're given bytes, make sure they fit into ASCII
    5:     if not isinstance(domain, text_type):
    5:         domain.decode('ascii')
    5:         return domain
    ...
    1: def _make_cookie_domain(domain):
           if domain is None:
               return None
